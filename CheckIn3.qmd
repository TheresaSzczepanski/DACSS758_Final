...
Math_DF_Train_Full<-Math_DF%>%
 ...
 mutate(id = row_number())%>%
  select(`doc_id`, `id`, `standard`, `reporting category`, `type`, `year`,  `modeling_factor`, `patterns_alg_factor`, `apply_division_factor`, `school_state_bool`, `school_state_diff`, `grade_level`)
...
Math_DF_Train_X<-Math_DF_Train%>%
  select( `standard`, `reporting category`, `type`, `year`,  `modeling_factor`, `patterns_alg_factor`, `apply_division_factor`, grade_level)
...
bool_RF<-randomForest(Math_DF_Train_X,
                      y = as.factor(Math_DF_Train$school_state_bool),
                      xtest = Math_DF_Test_X,
                      ytest = as.factor(Math_DF_Test$school_state_bool),
                      mtry = 2,
                      importance = TRUE,
                      ntree = 10000)
...
```{r}
# confusion matrix
actual <- as.factor(Math_DF_Test$school_state_bool)
predicted <- bool_RF$test[['predicted']]
...
#   filter(grade_level == 10 | grade_level == 8| grade_level == 7| grade_level == 6|grade_level == 5)%>%
  mutate(id = row_number())%>%
   select(`id`, `standard`, `reporting category`, `Cluster`, `text_type`, `year`, `school_state_diff`, `grade_level`)
# 
 ELA_DF_Train_Full
...
ELA_DF_Train_X<-ELA_DF_Train%>%
  select( `standard`, `reporting category`, `Cluster`, `text_type`, `year`, `grade_level`)
