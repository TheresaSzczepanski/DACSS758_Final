...
6.  Create a dictionary that uses key phrases identified in steps 4 and
    ...
    -   `analyze graph`: Students are asked questions involving the analysis
    of a graph
    
    -   `analyze model`: Students are asked questions involving the analysis
    of a model
...
7.  Update the `Subject_DF` to include the new features identified via
    ...
    feature for Supervised Machine Learning model that will predict `school_state_diff`.
...
of 0, and once using methods that includes removing stop words to use for text visualization and simple descriptive summaries of the text:
...
with the modeling problems. I will hopefully be able to explore this
with topic modeling in the future.
...
my_math_dict <- dictionary(list(modeling=c("real-world","context", "word_problem"),             
              patterns_alg=c("equivalent expression", "eqivalent", "distributive", "identify equivalent", "distribute", "factor", "parentheses"),
