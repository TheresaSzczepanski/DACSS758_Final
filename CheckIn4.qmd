...
When I examine the phrases in the first ten documents of weak items, there
...
-   Random Forest Regression Model including 2 additional text extracted features w/ parameters tuned with 5-fold
    ...
    -   The text–extracted feature: `poetry_factor` `figurative_language_factor` played a more significant role than the state provided variable `type` in the variable importance plot.
...
2.  I tried a SVM regression model; however because there are so many
    ...
    random forest for all of the tests without issue.
