...
tables, which is included in the Python collab notebook files linked
above.
...
2.  Using `preText`I looked for recommendations via the lowest score
    ...
    "or" play. These stop words do dominate the text desciptions though.
    Because of this, I also created a second set of tokens that included
    the removal of stopwords. Also, whenever there was a recommendation
    to use stemming, I instead used lemmatization
...
4.  Create visuals of the word connections for all item descriptions as
    ...
    stubsets to explore differences.
...
When I made vizualizations using the tokens that did not remove the stop
words, the stop words for so dominant. Thus for the remainder of my
visualizations I used my tokens that had stop words removed.
...
with the applied problems. I will hopefully be able to explore this with
